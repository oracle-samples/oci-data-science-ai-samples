...
"""Create an OCI Data Science model deployment on a Compute Target."""
...
from oci.data_science.models import (
    ...
    SingleModelConfigurationDetails,
...
LOGGER = logging.getLogger("create_model_deployment_on_compute_target")
...
def validate_sample_config(sample_config: Dict[str, Any]) -> None:
    ...
    model_deployment_config = sample_config["model_deployment"]
    require_value(model_deployment_config.get("project_id"), "model_deployment.project_id")
    require_value(model_deployment_config.get("model_id"), "model_deployment.model_id")
    require_value(
        model_deployment_config.get("compute_target_id"),
        "model_deployment.compute_target_id",
    )
    require_value(
        model_deployment_config.get("container_image"),
        "model_deployment.container_image",
    )

    request = model_deployment_config["resource_request"]
    limits = model_deployment_config.get("resource_limit")
    if limits:
        if limits["ocpus"] < request["ocpus"]:
            raise ValueError(
                "model_deployment.resource_limit.ocpus must be greater than or equal to resource_request.ocpus."
            )
        if limits["memory_in_gbs"] < request["memory_in_gbs"]:
            raise ValueError(
                "model_deployment.resource_limit.memory_in_gbs must be greater than or equal to resource_request.memory_in_gbs."
            )

    validate_scaling_config(model_deployment_config, "model_deployment.scaling")
    ...
    for index, rule in enumerate(autoscaling_config["rules"]):
        ...
        if rule_type in ("TARGET_CUSTOM_EXPRESSION", "CUSTOM"):
            ...
            if is_placeholder(rule["query"]):
                raise ValueError(
                    f"Replace the placeholder value for '{rule_field_name}.query' before running the sample."
                )
            if rule.get("metric_namespace") and is_placeholder(rule["metric_namespace"]):
                raise ValueError(
                    f"Replace the placeholder value for '{rule_field_name}.metric_namespace' before running the sample."
                )
            continue
        raise ValueError(
...
def build_resource_request(
    ...
    resource_request = model_deployment_config["resource_request"]
...
def build_workload_scaling_policy(
    ...
    if scaling_type != "AUTOSCALING":
        raise ValueError(
            "model_deployment.scaling.type must be FIXED_SIZE or AUTOSCALING."
        )
...
def build_create_model_deployment_details(
    sample_config: Dict[str, Any],
) -> CreateModelDeploymentDetails:
    model_deployment_config = sample_config["model_deployment"]
    return CreateModelDeploymentDetails(
        compartment_id=sample_config["compartment_id"],
        project_id=model_deployment_config["project_id"],
        display_name=model_deployment_config.get("display_name"),
        description=model_deployment_config.get("description"),
        model_deployment_configuration_details=SingleModelDeploymentFlexConfigurationDetails(
            model_configuration_details=SingleModelConfigurationDetails(
                model_id=model_deployment_config["model_id"]
            ),
            infrastructure_configuration_details=ManagedComputeClusterModelDeployInfrastructureConfigDetails(
                compute_target_id=model_deployment_config["compute_target_id"],
                model_deployment_resource_configuration=ManagedComputeClusterModelDeploymentResourceConfiguration(
                    resource_request_configuration=build_resource_request(
                        model_deployment_config
                    ),
                    resource_limit_configuration=build_resource_limit(
                        model_deployment_config
                    ),
                ),
            ...
            environment_configuration_details=OcirModelDeploymentEnvironmentConfigurationDetails(
                image=model_deployment_config["container_image"],
                image_digest=model_deployment_config.get("container_image_digest"),
                server_port=model_deployment_config.get("server_port", 8080),
                health_check_port=model_deployment_config.get(
                    "health_check_port", 8080
                ),
                environment_variables=model_deployment_config.get(
...
def print_payload(sample_config: Dict[str, Any]) -> None:
    create_details = build_create_model_deployment_details(sample_config)
    print(json.dumps(oci.util.to_dict(create_details), indent=2))


def create_model_deployment(sample_config: Dict[str, Any]) -> str:
    client = create_data_science_client(sample_config)
    composite_client = data_science.DataScienceClientCompositeOperations(client)

    try:
        response = composite_client.create_model_deployment_and_wait_for_state(
            create_model_deployment_details=build_create_model_deployment_details(
                sample_config
            ),
    ...
    if response.data.status != "SUCCEEDED":
        raise RuntimeError(
            "Model Deployment creation did not succeed. "
            f"Work request status: {response.data.status}"
        )

    model_deployment_id = get_work_request_resource_identifier(response.data)
    if not model_deployment_id:
        raise RuntimeError(
            "Model Deployment was created, but no resource OCID was returned."
        )

    LOGGER.info("Created Model Deployment: %s", model_deployment_id)
    return model_deployment_id


def main() -> None:
    parser = argparse.ArgumentParser(
        description="Create a model deployment on a Managed Compute Cluster Compute Target."
    )
    parser.add_argument(
        "--config-json",
        default="config.json",
    ...
    model_deployment_id = create_model_deployment(sample_config)

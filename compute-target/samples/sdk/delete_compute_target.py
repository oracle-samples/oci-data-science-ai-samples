...
"""Delete an OCI Data Science Compute Target."""
...
LOGGER = logging.getLogger("delete_compute_target")
...
def validate_sample_config(sample_config: Dict[str, Any]) -> None:
    delete_config = sample_config["compute_target_delete"]
    require_value(delete_config.get("compute_target_id"), "compute_target_delete.compute_target_id")
...
def print_payload(sample_config: Dict[str, Any]) -> None:
    ...
    print(
        json.dumps(
            {"compute_target_id": delete_config["compute_target_id"]},
            indent=2,
        )
    )


def delete_compute_target(sample_config: Dict[str, Any]) -> None:
    delete_config = sample_config["compute_target_delete"]
    client = create_data_science_client(sample_config)
    composite_client = data_science.DataScienceClientCompositeOperations(client)

    try:
        response = composite_client.delete_compute_target_and_wait_for_state(
            compute_target_id=delete_config["compute_target_id"],
            wait_for_states=["SUCCEEDED", "FAILED"],
        )
    except ServiceError:
        LOGGER.exception("Failed to delete Compute Target.")
        raise

    if response.data.status != "SUCCEEDED":
        raise RuntimeError(
            "Compute Target delete did not succeed. "
            f"Work request status: {response.data.status}"
        )

    LOGGER.info("Deleted Compute Target: %s", delete_config["compute_target_id"])


def main() -> None:
    parser = argparse.ArgumentParser(description="Delete a Compute Target.")
    parser.add_argument(
        "--config-json",
        default="config.json",
    ...
    parser.add_argument(
        "--yes",
        action="store_true",
        help="Confirm that the Compute Target should be deleted.",
    )
    ...
    if not args.yes:
        raise ValueError("Pass --yes to confirm Compute Target deletion.")

    delete_compute_target(sample_config)

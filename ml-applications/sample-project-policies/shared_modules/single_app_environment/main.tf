locals {
  environment_root_name = "ml-app-${var.application_name}-${var.environment_name}"
  parent_compartment_id_resolved = coalesce(var.parent_compartment_id, var.tenancy_id)
  external_subnet_compartment_id_resolved = coalesce(var.external_subnet_compartment_id, oci_identity_compartment.environment_root.id)
}
...
resource "oci_identity_compartment" "environment_root" {
  ...
  description = "Compartment for ML Application '${var.application_name}', for environment '${var.environment_name}'"
...
module "identity_ml_app_runtime_policies" {
  ...
  environment_name = var.environment_name
  external_subnet_compartment_id = local.external_subnet_compartment_id_resolved
...
module "identity_ml_app_operators" {
  ...
  depends_on = [oci_identity_compartment.environment_root]

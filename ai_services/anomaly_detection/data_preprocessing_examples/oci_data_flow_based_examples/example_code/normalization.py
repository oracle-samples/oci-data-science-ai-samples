...
def normalize_data(df, scaler_type, columns):
    """
    Scale numeric features using two methods 1) minmax normalization or 2) standardization
    Args:
        df: input dataframe
    ...
    return df.select(not_normalized_columns).join(scaledData, on="id").drop("id")
...
def main():
    ...
    columns = args.columns[0].split(" ") if len(args.columns) == 1 else args.columns
